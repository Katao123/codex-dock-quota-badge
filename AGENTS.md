...
8. A self-install into a running official Codex is allowed only after preflight passes and only through `./scripts/install.sh --allow-running`. Tell the user that Codex must be fully restarted afterward.
...
`preflight -> explain signature/permission impact -> install -> restart official Codex -> verify -> inspect Dock -> clean temporary copy`
